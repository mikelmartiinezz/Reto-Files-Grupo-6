...
public class MainReto {
    ...
    public static void main(String[] args) {
        ...
        do {
            ...
            switch (opc) {
                case 1: menuAnadir(); break;
                case 2: listarGruposPorNombre(); break;
                case 3: listarFestivalesPorNombre(); break;
                case 4: listarConciertosPorFecha(); break;
                case 5: eliminarFestival(); break;
                case 6: listarGruposDetallado(); break;
                case 7: consultarStaffPorDni(); break;
                case 8: modificarInformacion(); break;
    ...
    private static void menuAnadir() {
        ...
        do {
            ...
            opc = Utilidades.leerInt();
            switch (opc) {
                case 1: anadirGrupo(); break;
                case 2: anadirFestival(); break;
                case 3: anadirConciertoAFestival(); break;
    ...
    private static void anadirFestival() {
        try {
            ...
            String codFestival = nombreFestival.substring(0, 3).toUpperCase() + String.format("%02d", listaFestivales.size() + 1);
            ...
            do {
                ...
                String codConcierto = fecha.toString().replace("-", "") + listaGrupos.get(g1).getNombGrupo().substring(0,3).toUpperCase();
            ...
            try (ObjectOutputStream oos = new ObjectOutputStream(new FileOutputStream(ficheroFest))) {
                for (Festival fest : listaFestivales) oos.writeObject(fest);
            }
    ...
    private static void anadirConciertoAFestival() {
        try {
            ...
            try (ObjectInputStream ois = new ObjectInputStream(new FileInputStream(ficheroFest))) {
                while (true) {
                    try { listaFestivales.add((Festival) ois.readObject()); } catch(EOFException e){ break; }
                }
            }

            if (listaFestivales.isEmpty()) { System.out.println("No hay festivales."); return; }
            ...
            try (ObjectInputStream oisG = new ObjectInputStream(new FileInputStream(ficheroGrupos))) {
                while (true) {
                    try { listaGrupos.add((Grupo) oisG.readObject()); } catch(EOFException e){ break; }
                }
            }

            if (listaGrupos.size()<2) { System.out.println("Se necesitan al menos 2 grupos."); return; }

            char seguir = 0;
            do {
                System.out.println("Fecha del concierto (dd/MM/yyyy):");
                LocalDate fecha = Utilidades.leerFechaDMA();
                if (festival.getConciertos().containsKey(fecha)) { System.out.println("Error: ya existe un concierto en esa fecha."); continue; }

                // Mostrar grupos
                for (int i=0;i<listaGrupos.size();i++) System.out.println((i+1) + ". " + listaGrupos.get(i).getNombGrupo());
                System.out.println("Selecciona GRUPO PRINCIPAL:");
                int g1 = Utilidades.leerInt(1, listaGrupos.size())-1;

                int g2; do { System.out.println("Selecciona GRUPO INVITADO:"); g2=Utilidades.leerInt(1, listaGrupos.size())-1; } while(g1==g2);

                String codConcierto = fecha.toString().replace("-","") + listaGrupos.get(g1).getNombGrupo().substring(0,3).toUpperCase();
                festival.getConciertos().put(fecha,new Concierto(codConcierto,fecha,listaGrupos.get(g1),listaGrupos.get(g2)));
            ...
            try (ObjectOutputStream oos = new ObjectOutputStream(new FileOutputStream(ficheroFest))) {
                for (Festival f : listaFestivales) oos.writeObject(f);
            }

        } catch (Exception e) { System.out.println("Error al añadir el concierto."); e.printStackTrace(); }
    }
    ...
    private static void listarGruposDetallado() {
        ...
        try (ObjectInputStream ois = new ObjectInputStream(new FileInputStream(fichero))) {
            while (true) {
                try { listaGrupos.add((Grupo) ois.readObject()); } catch (EOFException e){ break; }
            }
        } catch (Exception e) {
    ...
    private static void consultarStaffPorDni() {
        ...
        if (!encontrado) System.out.println("No existe ningún staff con ese DNI.");
    }
 // -------------------- MODIFICAR CANTANTE --------------------
    private static void modificarCantante() {
        ...
        do {
            ...
            for (Persona p : grupoCantante.getPersonas().values()) {
                if (p instanceof Cantante && !p.getDni().equals(dni) &&
